...
export default {
  async fetch(req, env) {
    ...
    const sys = [
      ...
      'สำคัญมาก: ถ้ามีคำว่า "Methodology" มากกว่า 1 บล็อก หรือมีวิธีวิจัย/จำนวน N หลายชุด (เช่น Online N200, Online N300, F2F N200, F2F N300) ให้ถือว่าเป็นหลาย option เสมอ — 1 บล็อก = 1 option และตั้งชื่อ option ให้สื่อถึงวิธี+N (เช่น "Online Survey (N=200)")',
      'ทุก option (หรือกรณีทางเลือกเดียว) ต้องมี tasks อย่างน้อย 1 งานเสมอ ห้ามส่ง tasks ว่าง — งานร่วม (Development, Data Collection, QC, Analysis) ให้ใส่ในทุก option',
      'F2F Survey ใช้แรงงานเก็บข้อมูลมากกว่า Online และ N มากกว่าก็ใช้วันมากกว่า; จำนวนหัว RD (quanti) = N รวมของ option นั้น',
      'ถ้าโจทย์ระบุระยะเวลารวม (เช่น "10 weeks" / "10 สัปดาห์" / timeline) ให้วางแผนงานทั้งหมดให้พอดีกับจำนวนสัปดาห์นั้น และคิด manday ของแต่ละตำแหน่งในแต่ละงานให้สอดคล้องกับช่วงเวลา — งานที่กินหลายสัปดาห์ (เช่น Data Collection) ให้กระจาย manday ตามจำนวนสัปดาห์ที่ใช้จริง (เช่น เก็บข้อมูล 4 สัปดาห์ = ~20 วันทำงาน ต่อคนที่ลงเต็มเวลา) ทีมเก็บข้อมูลปกติคือ Panel Manager (คุมงาน ไม่กี่วัน/สัปดาห์) + Intern (ลงเก็บเต็มเวลา); manday รวมของ 1 ตำแหน่งไม่ควรเกินจำนวนวันทำงานที่มีในช่วงเวลานั้น เว้นแต่ใส่จำนวนคน (people) มากกว่า 1',
